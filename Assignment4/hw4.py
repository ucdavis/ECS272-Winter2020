...
app.layout = html.Div([
    ...
    html.Div([
        html.H4("Select maximum age of interest", style={"textAlign": "center"}),
        dcc.Slider(
            id='age_slider',
            min=response_data['Age'].min() + 2,
            max=response_data['Age'].max(),
            value=response_data['Age'].min()+5,
            marks={
                '17': {'label': '17'},
                '18': {'label': '18'},
                '19': {'label': '19'},
        ...
        html.Div([
            ...
            dcc.Dropdown(
                ...
                options=[
                    ...
                    {'label': '  Explore variables relates to Mathematics', 'value': 'snakes'}
                ],
                value='math',
                style=({"textAlign": "center"})
            ),
            html.H4('Select the number of principal components desired:', style=({"textAlign": "center"})),
            dcc.RadioItems(
                id='pca_radio',
    ...
    html.Div([
        html.H4('  '),
        html.H4('Figure on the top has 2 type of interaction: through slider bar, one can choose the maximum age of '
                'interest, through hovering on (mouse over), one can study the question of interest with regard to town'
                'or city. '),
        html.H4('The bottom left figure use Principal Component Analysis, which is a method used to reduce dimension.'
                'Since there are 150 variables for each data entry, we need to select only of variables of interest.'
                'Thus, only those with a correlation of 30% or higher to our variable/question of interest were '
                'selected and later used to compute the 2/3 principal components.'),
        html.H4(
            'The bottom right figure illustrated how demographic affect people\'s view toward movie / music through '
            'sankey graph.')
    ])
])
...
def update_pca_scatter(question_of_interest, hoverData, comp_num, ageValue):
    ...
    if comp_num == 2:
        ...
        return {
            ...
            'layout': go.Layout(title=f'2 Component analysis of question of interest with regard to ' + location,
                                height=500,
                                hovermode="closest",
    ...
    # 3 comp
        ...
        return {
            'data': trace,
            "layout": go.Layout(
                height=600, title=f"3 Component analysis of question of interest with regard to " + location,
